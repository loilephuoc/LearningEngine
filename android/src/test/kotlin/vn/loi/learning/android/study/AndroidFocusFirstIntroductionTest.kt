...
class AndroidFocusFirstIntroductionTest {
    ...
    fun `composition exposes direct rating and swipe Good on both Introduction sides`() {
        ...
        assertTrue(screen.contains("if (state is AndroidStudyState.Introduction && (!state.historyPreview || state.navigation.canCorrectRating)"))
        ...
        assertTrue(screen.contains("navigationEnabled = (state.revealed || focusedSkimUx || state.navigation.canPrevious) && !quickReviewTransitionPending"))
