...
class StudyCompactChromeTest {
    ...
    fun `production chrome keeps shared buttons callbacks semantics and one center scroll`() {
        ...
        assertEquals(2, Regex("""\.verticalScroll\(""").findAll(screen).count())
        assertTrue(screen.contains("if (bodyScrollEnabled) Modifier.verticalScroll(mainBodyScrollState)"))
        assertTrue(screen.contains("heightIn(max = 620.dp).verticalScroll(scroll)"))
