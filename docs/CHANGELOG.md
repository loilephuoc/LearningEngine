...
- Scalable Database Architecture: Migrated runtime persistence across root engine, Desktop (JVM), and Android from monolithic JSON files to SQLDelight 2.0.2 SQLite database (`learning_engine.db`) with full transactional safety, WAL mode, foreign keys, and indexes.
- Strict Platform Driver Separation: Implemented `SqliteDriverFactory` interface and pluggable driver provider architecture. Android exclusively uses `AndroidSqliteDriver` (`AndroidSqliteDriverFactory`) using Android's native `SQLiteOpenHelper`, strictly excluding JVM JDBC driver (`app.cash.sqldelight:sqlite-driver` and `org.xerial:sqlite-jdbc`) from the Android APK runtime. Desktop JVM uses `JvmSqliteDriverFactory` with JDBC SQLite.
- Lossless Automated JSON Migration: Automatically discovers legacy JSON stores (`contents.json`, `learning-items.json`, `content-libraries.json`, `content-packages.json`, `memory-states.json`, `review-events.json`, `learning-trajectories.json`, `study-sessions.json`, `study-queues.json`, `package-catalogs.json`, `installed-packages.json`, `canonical-libraries.json`, `canonical-library-collections.json`, `continuous-review-intents.json`, `knowledge-graph.json`, `sync-state.json`) on startup, migrates all records into SQLite within an atomic transaction, validates row counts, and preserves legacy JSON files on disk.
- Query Push-Down & Zero Hot-Path Memory Scans: Eliminated hot-path `loadAll()` into Kotlin memory for counting, budget calculation, and filtering. Implemented direct SQL queries with indexed WHERE, JOIN, COUNT, and LIMIT clauses for `DailyStudyBudgetQueryService`, due items, enabled items, and topic sessions.
- Full Backup & Interchange Compatibility: Maintained exact compatibility with OPD3 export/import formats, Format 1 local binary backups, and Format 2 portable snapshot backups with on-the-fly SQLite to JSON export.
- Cross-Platform Verification: Verified 100% test pass rates across root engine (2,357 tests), Desktop Compose (1,968 tests), and Android platform tests.
