...
class SqliteLocalSyncStateRepository(
    private val database: LearningEngineDatabase
) : LocalSyncStateRepository {

    private val queries = database.localSyncStateQueries
    ...
    private fun load(): LocalSyncStateRecord {
        val row = queries.get().executeAsOneOrNull() ?: return LocalSyncStateRecord().validated()
        return LocalSyncStateRecord(
            schemaVersion = 1,
            outbox = SqliteJsonUtils.decodeOrDefault(row.outboxJson, emptyList()),
            inbox = SqliteJsonUtils.decodeOrDefault(row.inboxJson, emptyList()),
            cursors = SqliteJsonUtils.decodeOrDefault(row.cursorsJson, emptyMap()),
            quarantines = SqliteJsonUtils.decodeOrDefault(row.quarantinesJson, emptyList()),
            pendingMedia = SqliteJsonUtils.decodeOrDefault(row.pendingMediaJson, emptyList()),
            mediaGcCandidates = SqliteJsonUtils.decodeOrDefault(row.mediaGcCandidatesJson, emptyList())
        ).validated()
    }
    ...
    private fun update(transform: (LocalSyncStateRecord) -> LocalSyncStateRecord) {
        database.transaction {
            ...
            queries.insertOrReplace(
                outboxJson = SqliteJsonUtils.encode(updated.outbox),
                inboxJson = SqliteJsonUtils.encode(updated.inbox),
                cursorsJson = SqliteJsonUtils.encode(updated.cursors),
                quarantinesJson = SqliteJsonUtils.encode(updated.quarantines),
                pendingMediaJson = SqliteJsonUtils.encode(updated.pendingMedia),
                mediaGcCandidatesJson = SqliteJsonUtils.encode(updated.mediaGcCandidates)
            )
        }
    }
}
