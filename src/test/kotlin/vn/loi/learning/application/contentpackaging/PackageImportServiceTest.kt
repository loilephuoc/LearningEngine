...
class PackageImportServiceTest {
    ...
    fun `import all scans imports persists and registers package`() {
        ...
        assertEquals(
            learningItem,
            learningItemRepository.findById(learningItem.id)
        )
        assertEquals(1, learningItemRepository.count())

        val persistedPackage =
            assertNotNull(
    ...
    @Test
    fun `import all imports multiple scanned packages in scanner order`() {
        val firstCandidate = PackageScanCandidate(
            source = "C:/packages/first.opd3"
        )

        val secondCandidate = PackageScanCandidate(
            source = "C:/packages/second.pkg"
        )

        val firstPackage = ContentPackage(
        ...
        val secondContent = Content(
            id = ContentId("content-second"),
            type = ContentType.WORD,
            text = ContentText(
                primaryText = "second",
                translatedText = "thu hai"
            )
        )

        val firstLearningItem = LearningItem(
            id = LearningItemId("item-first"),
            contentId = firstContent.id,
            mode = LearningMode.MEANING_RECOGNITION
        )

        val secondLearningItem = LearningItem(
            id = LearningItemId("item-second"),
            contentId = secondContent.id,
            mode = LearningMode.MEANING_RECOGNITION
        )

        val firstLibrary = ContentLibrary(
            id = ContentLibraryId("library-first"),
            descriptor = LibraryDescriptor("First Library"),
            contentIds = setOf(firstContent.id)
        )

        val secondLibrary = ContentLibrary(
            id = ContentLibraryId("library-second"),
            descriptor = LibraryDescriptor("Second Library"),
            contentIds = setOf(secondContent.id)
        )
        ...
        val installedSources = mutableListOf<String>()
        ...
        val service = PackageImportService(
            ...
            packageInstaller = PackageInstaller { candidate ->
                installedSources += candidate.source

                when (candidate) {
                    firstCandidate -> firstPackage
                    secondCandidate -> secondPackage
                    else -> error(
                        "Unexpected candidate: ${candidate.source}"
                    )
                }
            },
            packageContentImporter = PackageContentImporter { candidate ->
                ...
                when (candidate) {
                    ...
                    secondCandidate ->
                        ImportedPackageContent(
                            contents = listOf(secondContent),
                            learningItems = listOf(secondLearningItem),
                            libraries = listOf(secondLibrary)
                        )
        ...
        assertEquals(
            secondContent,
            contentRepository.findById(secondContent.id)
        )
        ...
        assertEquals(
            secondLearningItem,
            learningItemRepository.findById(secondLearningItem.id)
        )

        assertEquals(2, learningItemRepository.count())
        ...
        assertNotNull(
            contentPackageRepository.findById(secondPackage.id)
        )

        assertEquals(2, contentPackageRepository.count())
        ...
        assertTrue(catalog.contains(firstPackage.id))
        assertTrue(catalog.contains(secondPackage.id))
        assertEquals(2, catalog.packageCount)
    }
}